---
title: "Stage-structured clownfish harvesting model"
author: "E Schlatter"
date:
...
## Utility functions
Harvest runs one simulation for specified values of hf,hm,hj
```{r}
...
HarvestRun runs many simulations for sets of values of hf,hm,hj stored in matrix params
```{r}
...
For many harvest strategies, HarvestThreshold finds the threshold, i.e. the most conservative unsustainable strategy.
```{r}
...
Change F harvest rate and find persistence threshold
```{r}
...
Plot F harvest rate vs yield with persistence line
```{r}
par(mfrow=c(1,3))
...
Change M harvest rate and find persistence threshold
```{r}
...
Change NB harvest rate and find persistence threshold
```{r}
...
Utility functions to fit a beta distribution to data
```{r}
...
Get data on parameter values from literature sources, fit distributions to use in LHS. (Plot those distributions if desired)
```{r}
...
ECDF, scatterplot, PRCC plot w/significance lines
```{r}
par(mfrow=c(1,1))
...
Store PRCC output in usable form
```{r}
